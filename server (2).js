...
// 1. CREATE CHECKOUT SESSION (BESTEHENDER CODE)
...
app.post('/create-partner-analysis', async (req, res) => {
    try {
        ...
        // Generiere einzigartige ID für diese Analyse
        const analysisId = `partner_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        
        // Starte Analyse (GLEICHE Funktion wie bei Payment!)
        ...
        // Optional: Partner-Nutzung tracken
        // Du könntest hier in eine Datenbank schreiben:
        // await trackPartnerUsage(partnerCode, analysisId);
        
        // Sende Analyse SOFORT zurück (kein Warten wie bei Payment)
        res.json({ 
            status: 'complete',
            analysis: analysis,
...
// 2. ANALYZE WITH OPENAI GPT-4 (BESTEHENDER CODE - UNVERÄNDERT)
...
Formatiere die Antwort in strukturiertem HTML mit <h3> für Überschriften und <p>, <ul>, <li> für Inhalte. Sei spezifisch, konkret und realistisch!`;
        ...
        const completion = await openai.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: "Du bist ein erfahrener Karriere- und Studienberater mit 15+ Jahren Erfahrung. Du gibst konkrete, umsetzbare Empfehlungen basierend auf den individuellen Stärken und Interessen der Person."
                },
                {
                    role: "user",
                    content: prompt
...
// 3. GET ANALYSIS RESULT (BESTEHENDER CODE - UNVERÄNDERT)
