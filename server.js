...
`;
    ...
    const basisPrompt = `Erstelle eine persönliche HANDWERKS-Berufsanalyse auf Deutsch für folgende Person.
...
app.post('/lebenslauf-generator', rateLimit('lebenslauf', 60, HOUR), async (req, res) => {
    try {
        ...
        console.log(`📄 Lebenslauf-Generator angefragt: ${vorname} ${nachname} → ${zielberuf}${firmenname ? ' bei ' + firmenname : ''}`);
...
Antworte AUSSCHLIESSLICH mit einem validen JSON-Objekt in genau diesem Format, ohne Zusatztext, ohne Markdown-Codeblock:
{
  ...
  "anschreiben": {
    ...
    "text": "<vollständiger Anschreiben-Text mit Anrede, 3-4 Absätzen, Grußformel>"
        ...
        console.log(`✅ Lebenslauf & Anschreiben generiert: ${vorname} ${nachname}`);
        res.json(parsed);
    } catch (error) {
