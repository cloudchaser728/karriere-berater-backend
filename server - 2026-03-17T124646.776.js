...
const oneTimeCodes = new Map([
    ['POLITIK', { maxUses: 130, usedCount: 0 }],
    ['admin-daniel', { maxUses: 999999, usedCount: 0 }]
]);
    ...
    try {
        ...
        if (oneTimeCodes.has(partnerCode)) {
            ...
            if (codeData.usedCount >= codeData.maxUses) {
                console.log(`❌ Code erschöpft: ${partnerCode} (${codeData.usedCount}/${codeData.maxUses} verwendet)`);
            ...
            codeData.usedCount++;
            console.log(`✅ Code eingelöst: ${partnerCode} (${codeData.usedCount}/${codeData.maxUses})`); 
        }
...
Führe intern eine Liste aller bereits genannten Berufe – sowohl Hauptkarrierewege ALS AUCH Alternativen! (max. 9 Berufe gesamt: 3 Hauptwege + 6 Alternativen)

VERBOTEN: Ein Beruf der bereits als Hauptkariereweg ODER als Alternative genannt wurde, darf NIRGENDWO nochmal auftauchen!

Beispiel:
- Karriereweg 1: Landespolizist → Alternativen: Bundespolizist, Feuerwehrmann
...
✓ Sind alle 9 Berufe (3 Hauptwege + 6 Alternativen) einzigartig? → Ja? Dann weiter!
