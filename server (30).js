...
// ── STELLENSUCHE ENDPOINT (BA API) ──
app.post('/stellensuche', async (req, res) => {
    try {
        const { beruf, ort, umkreis = 25, arbeitszeit = '', angebotsart = '1', seite = 1 } = req.body;
        if (!beruf) return res.status(400).json({ error: 'Kein Beruf angegeben' });

        console.log(`🔍 Stellensuche: ${beruf} in ${ort || 'Deutschland'}`);
        ...
        if (ort) params.append('wo', ort);
        if (umkreis) params.append('umkreis', umkreis);
        if (arbeitszeit) params.append('arbeitszeit', arbeitszeit);

        const https = require('https');
        const data = await new Promise((resolve, reject) => {
            const options = {
                hostname: 'rest.arbeitsagentur.de',
                path: `/jobboerse/jobsuche-service/pc/v4/jobs?${params}`,
                method: 'GET',
        ...
        const stellen = (data.stellenangebote || []).map(s => ({
            titel: s.titel || 'Stelle ohne Titel',
            arbeitgeber: s.arbeitgeber || 'Unbekannter Arbeitgeber',
            ort: s.arbeitsort?.ort || ort || 'Deutschland',
            plz: s.arbeitsort?.plz || '',
            arbeitszeit: s.arbeitszeit || '',
            veroeffentlicht: s.eintrittsdatum || '',
            aktuelleVeroeffentlichungsdatum: s.aktuelleVeroeffentlichungsdatum || '',
            refnr: s.refnr || '',
            link: s.refnr ? `https://www.arbeitsagentur.de/jobsuche/jobdetail/${Buffer.from(s.refnr).toString('base64')}` : 'https://www.arbeitsagentur.de/jobsuche/'
        }));

        console.log(`✅ Stellensuche: ${stellen.length} Stellen gefunden`);
        res.json({ stellen, gesamt: data.maxErgebnisse || 0 });

    } catch (error) {
        console.error('Stellensuche Error:', error);
        res.status(500).json({ error: 'Fehler bei der Stellensuche' });
    }
});
...
app.listen(PORT, () => {
    ...
    console.log('🔍 Stellensuche-Endpoint aktiv!');
    console.log('=================================');
});
