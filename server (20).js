...
ERGEBNIS: {"beruf1":"...","beruf2":"...","beruf3":"...","hollandCode":"RIC"}` }
    ...
    const berufVorgabe12 = beruf1
        ? `Platz 1 = "${beruf1}", Platz 2 = "${beruf2}". Verwende GENAU diese Berufe.`
        : 'Wähle Platz 1 und Platz 2 selbst aus der Liste.';
    ...
    // ── SCHRITT 2: zwei Teile parallel erzeugen ──
...
🚨 JETZT NUR AUSGEBEN: Baustein A (Profil), Baustein B (Was wir in dir gesehen haben), Baustein C für Platz 1 und Platz 2.
${berufVorgabe12}
STOPP nach Platz 2.` }
            ]
        })),
...
${berufVorgabe3}
Profil und Platz 1/2 NICHT ausgeben – die kommen aus einem anderen Teil.` }
            ]
        }))
    ]);

    const draft = c1.choices[0].message.content + '\n' + c2.choices[0].message.content;

    // ── SCHRITT 3: Qualitätsprüfung ──
    let finalHtml = draft;
    try {
        const review = await callWithRetry(() => openai.chat.completions.create({
            model: "gpt-4o",
            temperature: 0.1,
            max_tokens: 16000,
            messages: [
                ...
                { role: "user", content: `Prüfe diese Handwerks-Analyse und gib NUR das korrigierte HTML zurück.
...
3. Jeder der 3 Berufe hat Gehaltstabelle UND Such-Buttons? → sonst ergänzen.
...
8. Meister-Gehalt niedriger als Geselle? → korrigieren. Bau-Tarif bei Nicht-Bau-Berufen (Dachdecker, Metallbauer, Kfz, Tischler, Maler, SHK, Elektro)? → durch passende Werte ersetzen. Match-Scores nicht absteigend? → anpassen. "Berufserfahrung vor dem Meister nötig"? → korrigieren (seit 2004 nicht mehr). Übrig gebliebene Anweisungstexte (z. B. "Konkrete ... nennen") → entfernen.
...
6. Markdown (##, **, \`\`\`) → in HTML umwandeln. Kaputtes HTML reparieren. Doppelte Abschnitte entfernen.

ANALYSE:
${draft}` }
            ]
        }));
        const reviewed = (review.choices[0].message.content || '').trim();
        if (reviewed.length > draft.length * 0.6) finalHtml = reviewed;
    } catch (e) {
        console.warn('⚠️ Handwerk-Review fehlgeschlagen, nutze Entwurf:', e.message);
    }
...
4. Karriereweg 2 – Beruf: "${beruf2}" (komplett mit Tabelle, Buttons, Steckbrief, Zukunftstrend, Warum, Alternativen)

WICHTIG: Verwende GENAU diese beiden vorgegebenen Berufe für Karriereweg 1 und 2 – wähle NICHT selbst einen anderen Beruf!
STOPP nach Karriereweg 2! Karriereweg 3, Weiterbildung und Nächste Schritte kommen aus einem separaten Teil.`;
...
NUR diese Sektionen ausgeben – Profil und Karriereweg 1 & 2 werden in einem separaten Teil erzeugt und dürfen NICHT wiederholt werden!`;
            ...
            const [completion1, completion2] = await Promise.all([
                callWithRetry(() => openai.chat.completions.create({
                    ...
                    messages: [
                        { role: "system", content: systemMessage },
                        { role: "user", content: prompt1Parallel }
                    ],
                    max_tokens: 11000,
                })),
                callWithRetry(() => openai.chat.completions.create({
                    ...
                    messages: [
                        { role: "system", content: systemMessage },
                        { role: "user", content: prompt2Parallel }
                    ],
                    max_tokens: 11000,
                }))
            ]);
