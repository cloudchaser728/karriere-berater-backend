...
// Middleware
app.set('trust proxy', 1); // Railway sitzt hinter einem Proxy – für echte Besucher-IP
app.use(cors());
...
// ═══════════════════════════════════════════════
// FEEDBACK-DATENBANK (PostgreSQL via Railway)
// ═══════════════════════════════════════════════
...
async function initFeedbackTable() {
    if (!dbPool) {
        console.warn('⚠️ DATABASE_URL nicht gesetzt – Feedback-Speicherung ist deaktiviert.');
        return;
    }
    try {
        await dbPool.query(`
        ...
        `);
        console.log('✅ Feedback-Tabelle bereit');
        ...
        `);
        console.log('✅ Sponsor-Click-Tabelle bereit');
        ...
        `);
        console.log('✅ Page-Visits-Tabelle bereit');
    } catch (err) {
        console.error('❌ Fehler beim Initialisieren der Tabellen:', err.message);
    }
}
initFeedbackTable();
...
// ═══════════════════════════════════════════════
// SICHERHEIT: Zahlungsprüfung, Partnercodes, Rate-Limit
// ═══════════════════════════════════════════════

// Fragebogen-Daten bezahlter Analysen: Stripe-Session-ID → { formData, started }
// Die KI-Analyse startet ERST, wenn Stripe die Zahlung bestätigt hat.
const pendingPaidAnalyses = new Map();

// Gültige Schul-/Partnercodes. In Railway als Variable PARTNER_CODES setzen,
// z. B. "TESTCODE,GYMNASIUM-DORTMUND,REALSCHULE-XY" (Groß-/Kleinschreibung egal).
const PARTNER_CODES = new Set(
    (process.env.PARTNER_CODES || 'TESTCODE')
        .split(',').map(c => c.trim().toUpperCase()).filter(Boolean)
);
function isValidPartnerCode(code) {
    if (!code) return false;
    const c = String(code).trim().toUpperCase();
    return PARTNER_CODES.has(c) || oneTimeCodes.has(c);
}

// Einfaches Rate-Limit pro IP (ohne Zusatzpaket). Schutz vor Bots, die KI-Kosten verursachen.
// Grenzen großzügig, weil ganze Schulklassen über dieselbe IP kommen können.
...
function rateLimit(name, max, windowMs) {
    return (req, res, next) => {
        ...
        if (hits.length >= max) {
            console.warn(`⛔ Rate-Limit ${name} für ${ip}`);
            return res.status(429).json({ error: 'Zu viele Anfragen', message: 'Bitte versuche es in ein paar Minuten erneut.' });
        }
        hits.push(now);
...
setInterval(() => {
    const now = Date.now();
    for (const [k, v] of rateBuckets) { if (!v.some(t => now - t < 3600000)) rateBuckets.delete(k); }
    for (const [k, v] of pendingPaidAnalyses) { if (now - v.createdAt > 24 * 3600000) pendingPaidAnalyses.delete(k); }
    for (const [k, t] of fehlgeschlageneAnalysen) { if (now - t > 24 * 3600000) fehlgeschlageneAnalysen.delete(k); }
    // Datenschutz: Analysen + Antworten spätestens nach 24 Stunden aus dem Speicher löschen
    for (const [k, v] of analysisResults) {
        const t = v && v.timestamp ? new Date(v.timestamp).getTime() : 0;
        if (!t || now - t > 24 * 3600000) analysisResults.delete(k);
    }
}, 600000);
const HOUR = 3600000;
...
const oneTimeCodes = new Map([
    ['ASR2026', { used: false, usedAt: null }],
    ['POLITIK', { used: false, usedAt: null }],
    ['DANIEL1403', { used: false, usedAt: null }]
]);

// Root route - serve HTML file from public folder
// Retry helper für OpenAI Rate Limits (429)
async function callWithRetry(fn, maxRetries = 7) {
    for (let i = 0; i < maxRetries; i++) {
        try {
            return await fn();
        } catch (err) {
            const retryable = err.status === 429 || err.status === 500 || err.status === 502 || err.status === 503;
            if (retryable && i < maxRetries - 1) {
                // Wartezeit wächst mit jedem Versuch (3s, 6s, 12s … max. 30s) – plus Vorgabe von OpenAI
                const vorgabe = err.headers?.['retry-after-ms'] ? parseInt(err.headers['retry-after-ms']) : 0;
                const waitMs = Math.max(vorgabe, Math.min(30000, 3000 * Math.pow(2, i))) + Math.floor(Math.random() * 1000);
...
app.post('/create-checkout-session', rateLimit('checkout', 30, HOUR), async (req, res) => {
    try {
        ...
        const session = await stripe.checkout.sessions.create({
            line_items: [{
                price_data: {
                    currency: 'eur',
                    product_data: {
                        name: (formData && formData.analyseTyp === 'handwerk') ? 'KI-Handwerk-Analyse' : 'KI-Karriereanalyse',
                        description: (formData && formData.analyseTyp === 'handwerk') ? 'Deine 3 passenden Handwerksberufe – persönlich mit KI ausgewertet' : 'Personalisierte Karriereberatung mit KI',
                    },
                    unit_amount: 999,
                },
                quantity: 1,
        ...
        // Analyse wird erst gestartet, wenn Stripe die Zahlung bestätigt (siehe /get-analysis)
        pendingPaidAnalyses.set(session.id, { formData, started: false, createdAt: Date.now() });
        analysisResults.delete(tempId);
...
// PARTNERCODE PRÜFEN (für schulen.html – Codes stehen nicht mehr im Browser)
app.post('/check-partner-code', rateLimit('codecheck', 30, HOUR), (req, res) => {
    const code = String((req.body && req.body.code) || '').trim().toUpperCase();
    if (!isValidPartnerCode(code)) return res.json({ valid: false });
    if (oneTimeCodes.has(code) && oneTimeCodes.get(code).used) return res.json({ valid: false, used: true });
    res.json({ valid: true, code: code });
});

// PARTNER-ANALYSE (KOSTENLOS)
app.post('/create-partner-analysis', rateLimit('partner', 60, HOUR), async (req, res) => {
    try {
        const { formData, partnerCode, source } = req.body;

        if (!isValidPartnerCode(partnerCode)) {
            console.warn(`❌ Ungültiger Partnercode abgelehnt: ${partnerCode}`);
            return res.status(403).json({ error: 'Ungültiger Code', message: 'Dieser Zugangscode ist nicht gültig.' });
        }
        ...
        // Neuer Ablauf: sofort antworten, die Seite fragt das Ergebnis dann über /get-analysis ab.
        // So klappt es auch, wenn eine ganze Klasse gleichzeitig startet und kurz gewartet werden muss.
        if (req.body.async === true) {
            analyzeCareerWithAI(formData, analysisId).catch(err => console.error('❌ Partner-Analyse Fehler:', err.message));
            return res.json({ status: 'processing', sessionId: analysisId });
        }
        
        const { analysis, hollandCode } = await analyzeCareerWithAI(formData, analysisId);
...
// ═══════════════════════════════════════════════════════════════
// HANDWERK-ANALYSE (eigener Fragebogen: /handwerk-analyse.html)
// Wird genutzt, wenn formData.analyseTyp === 'handwerk'.
// Speichert das Ergebnis im selben Format wie die normale Analyse,
// daher funktionieren /get-analysis, Stripe-Rückkehr & Anzeige unverändert.
// ═══════════════════════════════════════════════════════════════
const HANDWERK_BERUFSLISTE = `
⚡ ELEKTRO & ENERGIE:
Elektroniker/in – Energie- und Gebäudetechnik, Elektroniker/in – Automatisierungs- und Systemtechnik,
Elektroniker/in für Maschinen und Antriebstechnik, Informationselektroniker/in,
Elektroniker/in – Informations- und Telekommunikationstechnik

🚰 SANITÄR, HEIZUNG, KLIMA & KÄLTE:
Anlagenmechaniker/in für Sanitär-, Heizungs- und Klimatechnik, Mechatroniker/in für Kältetechnik,
Klempner/in, Ofen- und Luftheizungsbauer/in, Behälter- und Apparatebauer/in, Schornsteinfeger/in

🚗 FAHRZEUGE & MASCHINEN:
Kfz-Mechatroniker/in, Karosserie- und Fahrzeugbaumechaniker/in, Zweiradmechatroniker/in,
Land- und Baumaschinenmechatroniker/in, Fahrzeuglackierer/in

🧱 BAU & AUSBAU:
Maurer/in, Beton- und Stahlbetonbauer/in, Zimmerer/Zimmerin, Dachdecker/in, Straßenbauer/in,
Gerüstbauer/in, Fliesen-, Platten- und Mosaikleger/in, Estrichleger/in, Parkettleger/in,
Stuckateur/in, Brunnenbauer/in, Rollladen- und Sonnenschutzmechatroniker/in, Glaser/in

🪵 HOLZ & GESTALTUNG:
Tischler/in, Raumausstatter/in, Maler/in und Lackierer/in, Steinmetz/in und Steinbildhauer/in,
Bootsbauer/in

⚙️ METALL & TECHNIK:
Metallbauer/in, Feinwerkmechaniker/in, Konstruktionsmechaniker/in, Werkzeugmechaniker/in,
Zerspanungsmechaniker/in, Industriemechaniker/in, Mechatroniker/in, Goldschmied/in

🩺 GESUNDHEITSHANDWERK:
Augenoptiker/in, Hörakustiker/in, Zahntechniker/in, Orthopädietechnik-Mechaniker/in,
Orthopädieschuhmacher/in

🥖 LEBENSMITTEL:
Bäcker/in, Konditor/in, Fleischer/in

✂️ SCHÖNHEIT & MODE:
Friseur/in, Kosmetiker/in, Maßschneider/in
`;

function hwList(v) {
    if (v === undefined || v === null || v === '') return 'keine Angabe';
    return Array.isArray(v) ? v.join(', ') : String(v);
}

async function analyzeHandwerkWithAI(formData, sessionId) {
    console.log('🔧 Starte HANDWERK-Analyse für Session:', sessionId);
    const t0 = Date.now();
    const location = (formData.location || 'Deutschland').toString().slice(0, 80);
    const locationEncoded = encodeURIComponent(location);

    const profil = `
NUTZERPROFIL (Handwerk-Fragebogen):
- Alter: ${hwList(formData.age)}
- Aktuelle Situation: ${hwList(formData.situation)}
- Schulabschluss (vorhanden oder angestrebt): ${hwList(formData.schulabschluss)}
- Wohnort / PLZ: ${location}
- Führerschein Klasse B: ${hwList(formData.fuehrerschein)}
- Maximaler Pendelweg: ${hwList(formData.pendeln)}
- Gewünschter Ausbildungsstart: ${hwList(formData.start)}

ARBEITSUMFELD:
- Drinnen oder draußen: ${hwList(formData.umfeld)}
- Arbeiten in der Höhe: ${hwList(formData.hoehe)}
- Schmutz, Staub, Lärm: ${hwList(formData.schmutz)}
- Körperliche Arbeit: ${hwList(formData.koerper)}
- Kundenkontakt: ${hwList(formData.kunden)}
- Arbeitsweise: ${hwList(formData.team)}

INTERESSEN & STÄRKEN:
- Bereiche, die reizen: ${hwList(formData.bereiche)}
- Stärken: ${hwList(formData.staerken)}
- Lieblingsfächer in der Schule: ${hwList(formData.faecher)}
- Was er/sie in der Freizeit mit den Händen macht: ${hwList(formData.freizeit)}

NO-GOS (dürfen im empfohlenen Beruf NICHT zentral vorkommen!):
- ${hwList(formData.nogos)}

ZIELE:
- Was wichtig ist: ${hwList(formData.ziele)}
- Wunsch-Gewerk (falls angegeben): ${hwList(formData.wunsch)}
`;

    const systemMessage = `Du bist ein erfahrener Ausbildungsberater einer deutschen Handwerkskammer mit 15+ Jahren Erfahrung. Du kennst alle Handwerksberufe, ihren Alltag, ihre Vergütung und Karrierewege. Du berätst ehrlich, konkret und ermutigend – in der Du-Form, jugendgerecht, ohne Floskeln. Du gibst AUSSCHLIESSLICH sauberes HTML aus (kein Markdown, keine Codeblöcke).`;

    const basisPrompt = `${einfacheSpracheRegel(formData)}Erstelle eine persönliche HANDWERKS-Berufsanalyse auf Deutsch für folgende Person.

${profil}

📋 ERLAUBTE BERUFE – wähle NUR aus dieser Liste staatlich anerkannter Ausbildungsberufe:
${HANDWERK_BERUFSLISTE}

🚨 HARTE REGELN:
1. NUR Berufe aus der Liste oben. Keine Studiengänge, keine Büroberufe.
2. NO-GOS strikt beachten: Wer "Höhe" als No-Go hat, bekommt KEIN Dachdecker/Zimmerer/Gerüstbauer. Wer "Frühes Aufstehen / Nachtschicht" hat, bekommt KEIN Bäcker/in. Wer "Kälte & Nässe" hat, bekommt keinen überwiegenden Außenberuf. Usw.
3. Schulabschluss beachten: Für Handwerksberufe ist gesetzlich kein bestimmter Abschluss vorgeschrieben – aber sei ehrlich, wenn Betriebe bei einem Beruf meist einen mittleren Abschluss erwarten (z. B. Zahntechniker/in, Informationselektroniker/in).
4. Hat die Person (Fach-)Abitur, erwähne die Möglichkeit, die Ausbildung zu verkürzen.
5. Zahlen realistisch und als Spanne mit "ca." angeben. Ausbildungsvergütung je nach Tarif/Region. KEINE erfundenen Statistiken.
6. ❌ KEINE erfundenen Firmennamen oder Personen! Für Betriebe immer auf die Such-Buttons und die Handwerkskammer/Innung verweisen.
7. ❌ Keine Platzhalter in eckigen Klammern [ ] in der Ausgabe – alles mit echten Inhalten ersetzen.
8. Ehrlich sein: Pro Beruf auch sagen, was anstrengend ist.
10. MEISTERPRÜFUNG: Seit der Handwerksreform 2004 ist KEINE Berufserfahrung als Geselle mehr nötig – die Meisterschule kann direkt nach der Gesellenprüfung beginnen. NIEMALS "erst nach X Jahren Berufserfahrung" schreiben.
11. MATCH-SCORES: Platz 1 hat den höchsten Match-Wert, Platz 2 einen niedrigeren oder gleichen, Platz 3 den niedrigsten. Platz 3 darf nie höher sein als Platz 2.
12. Den Lebenslauf-Generator nur EINMAL erwähnen (im Tipp unter den nächsten Schritten), nicht zusätzlich als eigenen Schritt.
9. Aufstiegs-BAföG korrekt darstellen: 50 % der Lehrgangs- und Prüfungskosten als Zuschuss; der Rest als zinsgünstiges Darlehen, von dem bei bestandener Prüfung 50 % erlassen werden. Keine anderen Prozentzahlen erfinden.

💶 GEHALTS-ANKER 2026 – VERBINDLICH, NICHT DARUNTER GEHEN:
(Monatsbrutto = Stundenlohn × ca. 169 Std. bei 39-Std.-Woche bzw. × ca. 160 Std. bei 37 Std.)
• BAUGEWERBE (Maurer/in, Beton- und Stahlbetonbauer/in, Zimmerer/Zimmerin, Straßenbauer/in, Fliesen-, Platten- und Mosaikleger/in, Estrichleger/in, Stuckateur/in, Gerüstbauer/in, Brunnenbauer/in):
  Tarif ab 01.04.2026 bundesweit einheitlich (Ost = West): Facharbeiter/Geselle (Lohngruppe 3) 22,64 €/Std. + Bauzuschlag = 23,97 €/Std. → ca. 3.900–4.100 € brutto/Monat.
  Spezialfacharbeiter/Vorarbeiter-Niveau (Lohngruppe 4) 26,05 €/Std. inkl. Bauzuschlag → ca. 4.300–4.500 €.
  Ausbildungsvergütung Bau (Tarif 2026): 1. Jahr 1.122 €, 2. Jahr 1.351 €, 3. Jahr 1.610 € (4. Jahr 1.714 €).
• SHK (Anlagenmechaniker/in SHK, Klempner/in, Mechatroniker/in für Kältetechnik): Geselle 19–23 €/Std. → ca. 3.200–3.900 €; Tarif je nach Bundesland 15–22 €/Std., viele Betriebe zahlen übertariflich. Azubi je nach Region ca. 900–1.400 €.
• ELEKTRO: Geselle 19–23 €/Std. → ca. 3.200–3.900 €. Azubi ca. 900–1.300 €.
• DACHDECKER/IN, METALLBAUER/IN, KFZ-MECHATRONIKER/IN, TISCHLER/IN, MALER/IN: eigene Tarife, NICHT den Bau-Tarif verwenden! Geselle ca. 18–22 €/Std. → ca. 3.000–3.700 €, in Ballungsräumen/mit Erfahrung mehr.
• Ehrlich niedriger liegen: Friseur/in, Kosmetiker/in, Bäcker/in, Konditor/in, Fleischer/in (Geselle oft ca. 2.300–3.000 €) – hier NICHT schönrechnen, aber Aufstiegswege betonen.
• Meister/in angestellt: ca. 800–2.000 € über Gesellenniveau.
REGELN: Den Bau-Tarif (23,97 €/Std.) NUR für die oben unter BAUGEWERBE genannten Berufe verwenden. Meister/in (angestellt) muss IMMER höher liegen als Geselle/Gesellin desselben Berufs (Untergrenze Meister = Obergrenze Geselle). Ausbildungsvergütung für Nicht-Bau-Berufe realistisch je nach Gewerk angeben. Gib in der Gehaltstabelle IMMER zusätzlich den Stundenlohn als Geselle/Gesellin an. Für Bau-, SHK- und Elektro-Gesellen NIEMALS weniger als die Anker oben nennen – ein Einstieg unter 20 €/Std. ist dort unrealistisch niedrig. Lieber als Spanne "ca. X – Y €" angeben und kurz dazuschreiben: "mit Tarif, Überstunden und Zulagen oft mehr".

🎨 HTML-BAUSTEINE (genau diese Klassen verwenden):

A) PROFIL:
<div class="section-container">
  <h3>🔧 Dein Handwerker-Profil</h3>
  <p>3–4 Sätze: Wie arbeitest DU am liebsten, was treibt dich an – direkt aus den Antworten abgeleitet.</p>
</div>

B) "WAS WIR IN DIR GESEHEN HABEN" (Pflicht, direkt nach dem Profil):
<div class="section-container" style="background: linear-gradient(135deg, #3a2416 0%, #5c3a1e 100%); border-radius: 12px; padding: 24px; margin: 20px 0;">
  <h3 style="color: #e8c96a; margin-bottom: 16px;">🔍 Was wir in DIR gesehen haben</h3>
  <p style="color: #ffffff; font-size: 1.05em; line-height: 1.7;">4–6 Sätze. Mindestens 2 konkrete Antworten miteinander verknüpfen (z. B. Freizeit + Stärke). Wichtige Eigenschaften so hervorheben: <strong style="color:#ffffff;text-transform:uppercase;letter-spacing:0.5px;">PRÄZISE</strong>. Die No-Gos positiv wenden (Selbstkenntnis). Eine überraschende Beobachtung machen. Nicht generisch!</p>
  <p style="color: #e8c96a; font-weight: bold; margin-top: 12px; font-size: 0.95em;">💡 Diese Beobachtungen basieren ausschließlich auf DEINEN Antworten.</p>
</div>

C) PRO GEWERK (für jedes der 3 Gewerke komplett!):
<div class="career-path-card">
  <h3>🥇 Platz 1: BERUFSNAME <span style="font-size:0.8em;color:#5c3a1e;">– ca. XX % Match</span></h3>   (Platz 2 = 🥈, Platz 3 = 🥉)
  <p><strong>In einem Satz:</strong> Was man in diesem Beruf wirklich macht.</p>
  <h4>☀️ Ein typischer Tag</h4>
  <p>3–4 Sätze, bildhaft: Arbeitsbeginn, typische Aufgaben, Werkzeuge, wo man arbeitet.</p>
  <h4>✅ Warum das zu DIR passt</h4>
  <ul><li>3–4 Punkte mit direktem Bezug zu den Antworten</li></ul>
  <h4>💶 Das verdienst DU</h4>
  <table class="salary-table">
    <tr><th>Phase</th><th>Verdienst (brutto/Monat)</th></tr>
    <tr><td>1. Lehrjahr</td><td>ca. X €</td></tr>
    <tr><td>2. Lehrjahr</td><td>ca. X €</td></tr>
    <tr><td>3. Lehrjahr</td><td>ca. X €</td></tr>
    <tr class="highlight-row"><td>Als Geselle/Gesellin – Stundenlohn</td><td>ca. XX – XX €/Std.</td></tr>
    <tr class="highlight-row"><td>Als Geselle/Gesellin – Monat</td><td>ca. X.XXX – X.XXX €</td></tr>
    <tr class="highlight-row"><td>Als Meister/in (angestellt)</td><td>ca. X.XXX – X.XXX €</td></tr>
  </table>
  <p style="font-size:0.85em;color:#666;">Je nach Region, Tarifbindung und Betriebsgröße.</p>
  <div class="success-box">💪 <strong>Deine Karriereleiter:</strong> AUSBILDUNGSDAUER → Geselle/Gesellin → WEITERBILDUNG → ANGESTELLTE FÜHRUNGSROLLE ODER eigener Betrieb.</div>
  (In der Karriereleiter die Großbuchstaben-Platzhalter durch konkrete, zum Beruf passende Stationen ersetzen, z. B. bei Dachdecker/in: "Azubi (3 Jahre) → Geselle/Gesellin → Dachdeckermeister/in oder Bautechniker/in → Bauleiter/in ODER eigener Dachdeckerbetrieb". Diesen Hinweis selbst NICHT ausgeben.)
  <h4>⚖️ Ehrlicher Check</h4>
  <div class="info-box" style="background: white; border-left: 4px solid #3a2416;">
    <p><strong>💪 Das ist anstrengend:</strong> ehrlich, 1–2 Sätze.</p>
    <p><strong>🏠 Drinnen/Draußen · Höhe · Kundenkontakt:</strong> kurz einordnen und mit den Antworten abgleichen.</p>
    <p><strong>🎓 Voraussetzungen:</strong> Dauer der Ausbildung, üblicher Schulabschluss, ob Führerschein wichtig ist.</p>
  </div>
  <h4>🔮 Zukunft</h4>
  <div class="info-box" style="background: white; border-left: 4px solid #3a2416;">
    <p><strong>📈 Nachfrage:</strong> 🟢 SEHR HOCH / 🟢 HOCH / 🟡 STABIL + kurze Begründung.</p>
    <p><strong>🌍 Energiewende & Technik:</strong> Welche Rolle spielt der Beruf bei Klimaschutz, Digitalisierung, E-Mobilität (nur wenn wirklich zutreffend, sonst was sich technisch verändert).</p>
    <p><strong>🤖 KI-sicher?</strong> 1–2 Sätze, warum Hände und Erfahrung hier nicht ersetzbar sind – und welche digitalen Werkzeuge helfen.</p>
  </div>
  <h4>📍 Ausbildungsplätze & Praktika in ${location}:</h4>
  <div class="job-search-buttons">
    <a href="https://www.google.com/search?q=BERUF_URL+ausbildung+${locationEncoded}&ibp=htl;jobs" target="_blank" class="btn btn-accent" style="margin: 10px 5px; display: inline-block;">🔍 Google Jobs</a>
    <a href="https://www.ausbildung.de/suche?what=BERUF_URL&where=${locationEncoded}" target="_blank" class="btn" style="margin: 10px 5px; display: inline-block; background: #3b82f6;">📋 Ausbildung.de</a>
    <a href="https://www.arbeitsagentur.de/jobsuche/suche?was=BERUF_URL+Ausbildung&wo=${locationEncoded}" target="_blank" class="btn" style="margin: 10px 5px; display: inline-block; background: #1a4d2e;">🏛️ Arbeitsagentur</a>
    <a href="https://www.google.com/search?q=Praktikum+BERUF_URL+${locationEncoded}" target="_blank" class="btn" style="margin: 10px 5px; display: inline-block; background: #5c3a1e;">🧰 Praktikum finden</a>
  </div>
  (BERUF_URL = Berufsname URL-kodiert, ohne "/in", z. B. "Elektroniker" oder "Anlagenmechaniker+SHK")
</div>

D) WEG ZUM MEISTER:
<div class="section-container">
  <h3>🎖️ Dein Weg zum Meisterbrief</h3>
  <p>Für das Gewerk auf Platz 1 (Beruf beim Namen nennen): Die Meisterschule ist direkt nach der Gesellenprüfung möglich – Vollzeit ca. 1 Jahr, berufsbegleitend ca. 2–3 Jahre. Was man danach darf (Betrieb führen, ausbilden, Hochschulzugang ohne Abitur) und ungefährer Gehaltssprung in Euro.</p>
  <div class="success-box">💶 <strong>Aufstiegs-BAföG:</strong> 50 % der Lehrgangs- und Prüfungskosten gibt es als Zuschuss. Den Rest gibt es als zinsgünstiges Darlehen – bestehst du die Prüfung, werden davon noch einmal 50 % erlassen.</div>
</div>

E) NÄCHSTE SCHRITTE (genau 5, konkret, auf die 3 Gewerke bezogen):
<div class="section-container">
  <h3>🚀 Deine nächsten 5 Schritte</h3>
  <ol>
    <li><strong>Titel:</strong> konkrete Handlung (z. B. 2–3 Tage Praktikum in Gewerk X; Ausbildungsberatung der Handwerkskammer in der Region ${location} anrufen – die ist kostenlos; Bewerbungsunterlagen; Bewerbungszeitpunkt passend zum gewünschten Start; ggf. Führerschein)</li>
  </ol>
  <p>💡 Tipp: Bewerbungsunterlagen erstellst du schnell mit unserem <a href="/lebenslauf-generator.html">Lebenslauf- & Anschreiben-Generator</a>.</p>
</div>

F) FÜR DEINE ELTERN (kurz, Sie-Form):
<div class="info-box" style="background: #fbf6ee; border-left: 4px solid #c9a84c;">
  <p><strong>👨‍👩‍👧 Für Ihre Eltern:</strong> 3–4 Sätze in der Sie-Form: warum die empfohlenen Gewerke eine sichere Zukunft bieten (Fachkräftebedarf, frühes eigenes Einkommen, Meister/Studium später möglich) – sachlich, ohne Übertreibung.</p>
</div>
`;

    // ── SCHRITT 1: 3 Gewerke + Holland-Code vorab auswählen ──
    let beruf1 = null, beruf2 = null, beruf3 = null, hollandCode = null;
    try {
        const pre = await callWithRetry(() => openai.chat.completions.create({
            model: "gpt-4o",
            temperature: 0.1,
            max_tokens: 500,
            messages: [
                { role: "system", content: systemMessage },
                { role: "user", content: basisPrompt + `

🚨 DEINE EINZIGE AUFGABE JETZT:
Wähle GENAU 3 unterschiedliche, bestmöglich passende Berufe aus der Liste (verschiedene Gewerke, keine Synonyme). Beachte No-Gos, Arbeitsumfeld, Stärken, Interessen und Ziele. Ist ein Wunsch-Gewerk angegeben und passt es halbwegs, nimm es als einen der drei.
Bestimme außerdem den Holland-Code (RIASEC, 3 Buchstaben, stärkste Dimension zuerst).

Antworte so:
KURZE ÜBERLEGUNG: 2–4 Sätze
ERGEBNIS: {"beruf1":"...","beruf2":"...","beruf3":"...","hollandCode":"RIC"}` }
            ]
        }));
        const raw = pre.choices[0].message.content.trim();
        const after = raw.split(/ERGEBNIS\s*:/i).pop();
        const m = after.match(/\{[\s\S]*\}/);
        const parsed = JSON.parse(m ? m[0] : after);
        beruf1 = (parsed.beruf1 || '').trim();
        beruf2 = (parsed.beruf2 || '').trim();
        beruf3 = (parsed.beruf3 || '').trim();
        const hc = (parsed.hollandCode || '').trim().toUpperCase();
        if (/^[RIASEC]{3}$/.test(hc)) hollandCode = hc;
        const unique = new Set([beruf1, beruf2, beruf3].map(b => b.toLowerCase())).size;
        if (!beruf1 || !beruf2 || !beruf3 || unique !== 3) { beruf1 = beruf2 = beruf3 = null; }
        console.log(`🔧 Handwerk-Vorauswahl: ${beruf1} | ${beruf2} | ${beruf3} | ${hollandCode || 'n/a'}`);
    } catch (e) {
        console.warn('⚠️ Handwerk-Vorauswahl fehlgeschlagen, KI wählt selbst:', e.message);
    }

    const berufVorgabe1 = beruf1
        ? `Platz 1 = "${beruf1}". Verwende GENAU diesen Beruf.`
        : 'Wähle Platz 1 selbst aus der Liste.';
    const berufVorgabe2 = beruf1
        ? `Platz 2 = "${beruf2}". Verwende GENAU diesen Beruf. (Platz 1 ist "${beruf1}", Platz 3 ist "${beruf3}".)`
        : 'Wähle für Platz 2 einen passenden Beruf aus der Liste.';
    const berufVorgabe3 = beruf1
        ? `Platz 3 = "${beruf3}". Verwende GENAU diesen Beruf. (Platz 1 ist "${beruf1}", Platz 2 ist "${beruf2}".)`
        : 'Wähle für Platz 3 einen passenden Beruf aus der Liste, der sich klar von typischen Top-Empfehlungen unterscheidet.';

    // ── SCHRITT 2: drei Teile parallel erzeugen (kürzere Antworten = schneller) ──
    const hwGen = (aufgabe) => callWithRetry(() => openai.chat.completions.create({
        model: "gpt-4o",
        temperature: 0.2,
        max_tokens: 5500,
        messages: [
            { role: "system", content: systemMessage },
            { role: "user", content: basisPrompt + aufgabe }
        ]
...
🚨 JETZT NUR AUSGEBEN: Baustein A (Profil), Baustein B (Was wir in dir gesehen haben) und Baustein C für Platz 1.
${berufVorgabe1}
STOPP nach Platz 1.`),
        hwGen(`

🚨 JETZT NUR AUSGEBEN: Baustein C für Platz 2.
${berufVorgabe2}
Profil, Platz 1, Platz 3 und alle weiteren Bausteine NICHT ausgeben – die kommen aus anderen Teilen.`),
        hwGen(`

🚨 JETZT NUR AUSGEBEN: Baustein C für Platz 3, danach Baustein D (Weg zum Meister, bezogen auf Platz 1), Baustein E (Nächste Schritte) und Baustein F (Für deine Eltern).
${berufVorgabe3}
Profil und Platz 1/2 NICHT ausgeben – die kommen aus anderen Teilen.`)
    ]);

    // ── SCHRITT 3: Qualitätsprüfung – alle Teile PARALLEL ──
    const hwReviewPrompt = (teil) => `Prüfe diesen TEIL einer Handwerks-Analyse und gib NUR diesen Teil korrigiert als HTML zurück (keine anderen Abschnitte ergänzen).

NO-GOS DER PERSON: ${hwList(formData.nogos)}
${einfacheSpracheRegel(formData) ? 'EINFACHE SPRACHE gewählt: kurze Sätze und einfache Wörter beibehalten bzw. herstellen!' : ''}
ORT: ${location}

PFLICHT-CHECKS:
1. Platzhalter in [ ] oder "BERUF_URL" übrig? → durch echte Werte ersetzen.
2. Widerspricht ein empfohlener Beruf einem No-Go? → Hinweis im Ehrlichen Check ergänzen, dass/wie man damit umgehen kann.
3. Hat jeder Beruf in diesem Teil Gehaltstabelle UND Such-Buttons? → sonst ergänzen.
4. Erfundene Firmennamen? → entfernen und stattdessen auf Handwerkskammer/Such-Buttons verweisen.
5. Aufstiegs-BAföG korrekt (50 % Zuschuss, 50 % Erlass des Restdarlehens bei Bestehen)? → sonst korrigieren.
6. Markdown (##, **, \`\`\`) → in HTML umwandeln. Kaputtes HTML reparieren.
7. Gehälter zu niedrig? Bau-Gesellen (Maurer, Fliesenleger, Zimmerer, Beton, Straßenbau, Estrich, Stuckateur) nach Tarif 2026 ca. 23,97 €/Std. bzw. 3.900–4.100 €/Monat; SHK & Elektro 19–23 €/Std. bzw. 3.200–3.900 €. Werte darunter → anheben. Stundenlohn-Zeile fehlt → ergänzen.
8. Meister-Gehalt niedriger als Geselle? → korrigieren. Bau-Tarif bei Nicht-Bau-Berufen (Dachdecker, Metallbauer, Kfz, Tischler, Maler, SHK, Elektro)? → durch passende Werte ersetzen. "Berufserfahrung vor dem Meister nötig"? → korrigieren (seit 2004 nicht mehr). Übrig gebliebene Anweisungstexte (z. B. "Konkrete ... nennen") → entfernen.

TEIL:
${teil}`;

    const reviewed = await Promise.all(drafts.map(teil => callWithRetry(() => openai.chat.completions.create({
        model: "gpt-4o",
        temperature: 0.1,
        max_tokens: 5500,
        messages: [
            { role: "system", content: "Du bist ein präziser HTML-Qualitätsprüfer für Berufsanalysen. Du gibst AUSSCHLIESSLICH korrigiertes HTML zurück – keine Erklärungen, kein Markdown." },
            { role: "user", content: hwReviewPrompt(teil) }
        ]
    })).then(r => {
        const out = (r.choices[0].message.content || '').trim();
        return out.length > teil.length * 0.6 ? out : teil;
    }).catch(e => {
        console.warn('⚠️ Handwerk-Review eines Teils fehlgeschlagen, nutze Entwurf:', e.message);
        return teil;
    })));
    const finalHtml = reviewed.join('\n');
    ...
    console.log(`✅ Handwerk-Analyse fertig (${Date.now() - t0}ms) für Session: ${sessionId}`);
    return { analysis: finalHtml, hollandCode: hollandCode };
}


// Option „Ergebnis in einfacher Sprache“ (für schwächere Leser, z. B. Haupt- und Förderschulen)
function einfacheSpracheRegel(formData) {
    const v = formData && formData.einfacheSprache;
    if (!(v === true || v === 'true' || v === 'ja' || v === 'on')) return '';
    return `🟦 PFLICHT – EINFACHE SPRACHE:
Die Person hat "Ergebnis in einfacher Sprache" gewählt. Schreibe die GESAMTE Ausgabe in einfacher Sprache:
- Kurze Sätze (höchstens ca. 12 Wörter), ein Gedanke pro Satz.
- Einfache, bekannte Wörter. Fachwörter nur, wenn nötig – dann direkt in Klammern kurz erklären.
- Keine Schachtelsätze, keine Fremdwörter, keine Abkürzungen ohne Erklärung.
- Aktiv statt Passiv. Die Person direkt mit "du" ansprechen.
- Zahlen als Ziffern. Wichtige Punkte als kurze Liste.
- Struktur, Tabellen und Buttons bleiben vollständig erhalten – nur die Sprache wird einfacher.

`;
}

// ═══════════════════════════════════════════════
// WARTESCHLANGE: nur so viele Analysen gleichzeitig, wie das OpenAI-Limit schafft.
// Weitere warten automatisch und starten, sobald Platz frei ist – niemand bekommt einen Fehler.
// Kapazität per Railway-Variable ANALYSE_KAPAZITAET anpassbar (Standard 6 → z. B. 3 Karriere- oder 6 Handwerk-Analysen gleichzeitig).
// ═══════════════════════════════════════════════
const ANALYSE_KAPAZITAET = Math.max(1, parseInt(process.env.ANALYSE_KAPAZITAET || '6', 10));
let analyseBelegt = 0;
const analyseWarteliste = [];
function analyseGewicht(formData) { return (formData && formData.analyseTyp === 'handwerk') ? 1 : 2; }
function analyseSlotHolen(gewicht) {
    return new Promise(resolve => {
        const versuch = () => {
            if (analyseBelegt + gewicht <= ANALYSE_KAPAZITAET) { analyseBelegt += gewicht; resolve(); return true; }
            return false;
        };
        if (!versuch()) {
            analyseWarteliste.push(versuch);
            console.log(`⏳ Analyse wartet in der Warteschlange (Position ${analyseWarteliste.length})`);
        }
    });
}
function analyseSlotFreigeben(gewicht) {
    analyseBelegt = Math.max(0, analyseBelegt - gewicht);
    // Wartende der Reihe nach starten, solange Platz ist
    while (analyseWarteliste.length) {
        if (analyseWarteliste[0]()) analyseWarteliste.shift(); else break;
    }
}
const fehlgeschlageneAnalysen = new Map(); // sessionId → Zeitpunkt

// ANALYZE WITH OPENAI GPT-4
async function analyzeCareerWithAI(formData, sessionId) {
    const gewicht = analyseGewicht(formData);
    await analyseSlotHolen(gewicht);
    try {
        fehlgeschlageneAnalysen.delete(sessionId);
        return await analyzeCareerWithAIIntern(formData, sessionId);
    } catch (e) {
        fehlgeschlageneAnalysen.set(sessionId, Date.now());
        throw e;
    } finally {
        analyseSlotFreigeben(gewicht);
    }
}

async function analyzeCareerWithAIIntern(formData, sessionId) {
    // Eigener Handwerk-Fragebogen → eigene Auswertung
    if (formData && formData.analyseTyp === 'handwerk') {
        return analyzeHandwerkWithAI(formData, sessionId);
    }
    try {
        ...
        // ═══════════════════════════════════════════════
        // SCORE-/MATCHING-VORFILTER: Berufsliste automatisch
        // nach den vom User gewählten Interessen filtern,
        // BEVOR der Prompt an die KI geht.
        // ═══════════════════════════════════════════════
        ...
        function filterBerufslisteByInterests(text, rawInterests) {
            if (!rawInterests) return text; // keine Angabe -> volle Liste
            const interests = Array.isArray(rawInterests) ? rawInterests : [rawInterests];
            if (interests.length === 0) return text;

            const emojiPattern = Object.keys(CATEGORY_TAG_MAP).join('|');
            const splitRegex = new RegExp(`\\n(?=(?:${emojiPattern}) )`);
            const parts = text.split(splitRegex);

            // Erster Teil ist der Blocktitel (═══...UNI-STUDIENGÄNGE...═══), immer behalten
            const header = parts[0];
            const categoryParts = parts.slice(1);

            const kept = categoryParts.filter(part => {
                const emoji = Object.keys(CATEGORY_TAG_MAP).find(e => part.startsWith(e));
                if (!emoji) return true; // unbekanntes Format -> sicherheitshalber behalten
                return interests.includes(CATEGORY_TAG_MAP[emoji]);
            });

            // Fallback: falls Filter aus Versehen alles rauswirft, volle Liste zurückgeben
            if (kept.length === 0) return text;

            // Sicherheitsnetz: bei zu wenigen Kategorien (z.B. nur 1 Interesse gewählt)
            // hat die KI zu wenig Auswahl für 3 unterschiedliche Karrierewege.
            // Dann lieber mehr durchlassen als die KI mit einer zu dünnen Liste zu überfordern.
            if (kept.length < 3) return text;
        ...
        // Vorsortierung anwenden: nur Kategorien behalten, die zu den gewählten Interessen passen
        berufsliste = filterBerufslisteByInterests(berufsliste, formData.interests);

        // ═══════════════════════════════════════════════
        // HANDWERK-MODUS: Falls gesetzt, Berufsliste auf
        // NUR die Handwerksberufe einschränken (überschreibt
        // die normale Bildungsweg-Auswahl, da Handwerk = Ausbildung)
        // ═══════════════════════════════════════════════
        if (formData.handwerkModus === true || formData.handwerkModus === 'true') {
            const handwerkMatch = berufsliste.match(/🔧 HANDWERK:\n([\s\S]*?)(?=\n\n🌿|\n\n🛡️|\n\n✈️|$)/);
            if (handwerkMatch) {
                berufsliste = '═══════════════════════════════════════════════\n🔧 HANDWERKSBERUFE (Ausbildung)\n═══════════════════════════════════════════════\n\n🔧 HANDWERK:\n' + handwerkMatch[1];
            }
        }

        const prompt = `${einfacheSpracheRegel(formData)}DU bist ein professioneller Karriere- und Studienberater. Analysiere folgende Informationen und erstelle eine detaillierte, personalisierte Karriereberatung auf Deutsch.
...
STOPP nach Karriereweg 2! Karriereweg 3, Weiterbildung und Nächste Schritte kommen im nächsten Schritt.`;
        ...
        // ═══════════════════════════════════════════════
        // VORAB-BERUFSAUSWAHL: schneller, kleiner KI-Call,
        // der nur 3 unterschiedliche Berufsnamen auswählt.
        // Ermöglicht es, Call 1 & 2 danach PARALLEL laufen
        // zu lassen (statt nacheinander), ohne Qualitäts-
        // verlust bei der Berufsauswahl.
        // ═══════════════════════════════════════════════
        const t0 = Date.now();
        let beruf1 = null, beruf2 = null, beruf3 = null, preSelectOk = false, hollandCode = null;

        try {
            const preSelectPrompt = prompt + `

🛑🛑🛑 BILDUNGSWEG-PFLICHTCHECK 🛑🛑🛑
${bildungswegRegel}
🛑🛑🛑

🚨 DEINE EINZIGE AUFGABE JETZT:
1. Wähle GENAU 3 unterschiedliche, zum Profil bestmöglich passende Berufe/Studiengänge aus der Berufsliste oben aus – keine Wiederholungen, keine Synonyme desselben Berufs.
Berücksichtige dabei sorgfältig: Interessen, Persönlichkeit (Holland-Code-Auswertung), die Bildungsweg-Regel oben, den Flow-Zustand, und alle genannten No-Gos.
2. Bestimme zusätzlich den Holland-Code (RIASEC) des Profils: die 3 stärksten der 6 Dimensionen R (Realistisch/praktisch), I (Investigativ/forschend), A (Künstlerisch/kreativ), S (Sozial/helfend), E (Unternehmerisch/führend), C (Konventionell/organisierend) – als 3 Buchstaben, stärkste zuerst, z.B. "RIC".

Denk dir kurz (2-4 Sätze) durch, welche Berufe basierend auf dem Profil am besten passen und warum, und welcher Holland-Code sich daraus ergibt – das hilft dir, die wirklich besten 3 zu finden statt naheliegende Standardvorschläge zu wählen. Danach folgt dein finales Ergebnis.

Antworte in diesem Format:
KURZE ÜBERLEGUNG: [2-4 Sätze, warum genau diese 3 Berufe und dieser Holland-Code am besten zum Profil passen]
ERGEBNIS: {"beruf1":"<Beruf>","beruf2":"<Beruf>","beruf3":"<Beruf>","hollandCode":"<3 Buchstaben, z.B. RIC>"}

Das JSON-Objekt nach "ERGEBNIS:" muss exakt diesem Format entsprechen – keine zusätzlichen Felder, keine Markdown-Codeblöcke um das JSON.`;

            const preSelectCompletion = await callWithRetry(() => openai.chat.completions.create({
                model: "gpt-4o",
                temperature: 0.1,
                messages: [
                    { role: "system", content: systemMessage },
                    { role: "user", content: preSelectPrompt }
                ],
                max_tokens: 500,
            }));

            const raw = preSelectCompletion.choices[0].message.content.trim();
            const ergebnisSplit = raw.split(/ERGEBNIS\s*:/i);
            const searchText = ergebnisSplit.length > 1 ? ergebnisSplit[ergebnisSplit.length - 1] : raw;
            const jsonMatch = searchText.match(/\{[\s\S]*\}/);
            const parsed = JSON.parse(jsonMatch ? jsonMatch[0] : raw);
            beruf1 = (parsed.beruf1 || '').trim();
            beruf2 = (parsed.beruf2 || '').trim();
            beruf3 = (parsed.beruf3 || '').trim();
            const hollandCodeRaw = (parsed.hollandCode || '').trim().toUpperCase();
            if (/^[RIASEC]{3}$/.test(hollandCodeRaw)) {
                hollandCode = hollandCodeRaw;
            }

            const uniqueCount = new Set([beruf1, beruf2, beruf3].map(b => b.toLowerCase())).size;
            if (beruf1 && beruf2 && beruf3 && uniqueCount === 3) {
                preSelectOk = true;
                console.log(`✅ Vorab-Berufsauswahl (${Date.now() - t0}ms): ${beruf1} | ${beruf2} | ${beruf3} | Holland-Code: ${hollandCode || 'n/a'}`);
            } else {
                console.warn('⚠️ Vorab-Berufsauswahl unvollständig/doppelt, falle zurück auf klassischen Ablauf.');
            }
        } catch (e) {
            console.warn('⚠️ Vorab-Berufsauswahl fehlgeschlagen, falle zurück auf klassischen Ablauf:', e.message);
        }

        const t1 = Date.now();
        let part1, part2, parts = null;

        if (preSelectOk) {
            // ── PARALLELER PFAD: Berufe sind bereits vorgegeben ──
            const prompt1Parallel = prompt + `
...
1. DEIN PROFIL
2. "Was wir in DIR gesehen haben" Block
3. Karriereweg 1 – Beruf: "${beruf1}" (komplett mit Tabelle, Buttons, Steckbrief, Zukunftstrend, Warum, Alternativen)

WICHTIG: Verwende GENAU diesen vorgegebenen Beruf für Karriereweg 1 – wähle NICHT selbst einen anderen Beruf!
STOPP nach Karriereweg 1! Karriereweg 2, Karriereweg 3, Weiterbildung und Nächste Schritte kommen aus separaten Teilen.`;

            // Karriereweg 2 als eigener, paralleler Teil → kürzere Einzelantworten = schneller
            const prompt1bParallel = prompt + `

🛑🛑🛑 BILDUNGSWEG-PFLICHTCHECK VOR AUSGABE 🛑🛑🛑
${bildungswegRegel}
🛑🛑🛑

🚨 DEINE AUFGABE – NUR FOLGENDES AUSGEBEN:
Karriereweg 2 – Beruf: "${beruf2}" (komplett mit Tabelle, Buttons, Steckbrief, Zukunftstrend, Warum, Alternativen)

WICHTIG: Verwende GENAU "${beruf2}" – wähle NICHT selbst einen anderen Beruf! Die anderen Karrierewege sind "${beruf1}" und "${beruf3}" – nenne sie NICHT als Alternativen.
Profil, "Was wir in DIR gesehen haben", Karriereweg 1 und 3, Weiterbildung und Nächste Schritte werden in separaten Teilen erzeugt und dürfen hier NICHT ausgegeben werden!`;

            const prompt2Parallel = prompt + `

🚨 DEINE AUFGABE – NUR FOLGENDES AUSGEBEN:
1. Karriereweg 3 – Beruf: "${beruf3}" (komplett mit Tabelle, Buttons, Steckbrief, Zukunftstrend, Warum, Alternativen)
2. Uni/Hochschul-Empfehlungen (falls relevant für den Bildungsabschluss)
3. Weiterbildungs-Tipps für alle 3 Karrierewege: "${beruf1}", "${beruf2}" und "${beruf3}"
4. Konkrete Nächste Schritte (5 Schritte mit lokalen Firmennamen aus ${location}) bezogen auf diese 3 Berufe

WICHTIG: Verwende GENAU "${beruf3}" für Karriereweg 3 – wähle NICHT selbst einen anderen Beruf!
🛑🛑🛑 BILDUNGSWEG-PFLICHTCHECK – AUCH FÜR KW3 🛑🛑🛑
${bildungswegRegel}
🛑🛑🛑

NUR diese Sektionen ausgeben – Profil und Karriereweg 1 & 2 werden in einem separaten Teil erzeugt und dürfen NICHT wiederholt werden!`;

            const gen = (userPrompt) => callWithRetry(() => openai.chat.completions.create({
                model: "gpt-4o",
                temperature: 0.1,
                messages: [
                    { role: "system", content: systemMessage },
            ...
            const [completion1, completion1b, completion2] = await Promise.all([
                gen(prompt1Parallel), gen(prompt1bParallel), gen(prompt2Parallel)
            ]);

            parts = [
                completion1.choices[0].message.content,
                completion1b.choices[0].message.content,
                completion2.choices[0].message.content
            ];
            part1 = parts[0] + '\n' + parts[1];
            part2 = parts[2];
            console.log(`✅ Call 1 & 2 parallel fertig (${Date.now() - t1}ms), starte Review-Loop (Call 3)...`);

        } else {
            // ── KLASSISCHER SEQUENZIELLER PFAD (Fallback) ──
            const prompt1 = prompt + `
...
STOPP nach Karriereweg 2! Karriereweg 3, Weiterbildung und Nächste Schritte kommen im nächsten Schritt.`;
            ...
            part1 = completion1.choices[0].message.content;
            console.log(`✅ Call 1 fertig (${Date.now() - t1}ms), starte Call 2...`);
            const t1b = Date.now();
...
NUR diese Sektionen ausgeben – Profil und Karriereweg 1 & 2 wurden bereits in Teil 1 ausgegeben und dürfen NICHT wiederholt werden!`;
            ...
            part2 = completion2.choices[0].message.content;
            console.log(`✅ Call 2 fertig (${Date.now() - t1b}ms), starte Review-Loop (Call 3)...`);
        }

        const t2 = Date.now();

        // ── CALL 3: REVIEW-LOOP – KI prüft die kombinierte Analyse ──
        const reviewTeile = parts || [part1, part2];
        const buildReviewPrompt = (teil) => `Prüfe diese Karriereanalyse und gib NUR das korrigierte HTML zurück – keine Erklärungen.
...
${einfacheSpracheRegel(formData) ? 'WICHTIG: Die Person hat EINFACHE SPRACHE gewählt – kurze Sätze und einfache Wörter beibehalten bzw. herstellen!\n' : ''}PFLICHT-CHECKS:
...
HINWEIS: Dies ist nur EIN TEIL der Analyse – gib genau diesen Teil korrigiert zurück, ergänze keine anderen Abschnitte.

ZU PRÜFENDER TEIL:
${teil}`;

        // Teile PARALLEL prüfen statt alles auf einmal → deutlich schneller
        const reviewErgebnisse = await Promise.all(reviewTeile.map(teil => callWithRetry(() => openai.chat.completions.create({
            model: "gpt-4o",
            temperature: 0.1,
            messages: [
                {
                ...
                { role: "user", content: buildReviewPrompt(teil) }
            ],
            max_tokens: 7000,
        })).then(r => {
            const out = (r.choices[0].message.content || '').trim();
            ...
            console.warn('⚠️ Review eines Teils fehlgeschlagen, nutze Entwurf:', e.message);
            return teil;
        })));

        const reviewedAnalysis = reviewErgebnisse.join('\n');
        console.log(`✅ Review-Loop (Call 3) fertig (${Date.now() - t2}ms) – finale Analyse bereit! Gesamt: ${Date.now() - t0}ms`);
        ...
        return { analysis: reviewedAnalysis, hollandCode: hollandCode };
    } catch (error) {
...
app.get('/get-analysis/:sessionId', rateLimit('getanalysis', 600, HOUR), async (req, res) => {
    const { sessionId } = req.params;
    
    try {
        // Bezahlte Analysen (Stripe-Session "cs_...") nur nach bestätigter Zahlung herausgeben
        if (sessionId.startsWith('cs_')) {
            const pending = pendingPaidAnalyses.get(sessionId);
            const existing = analysisResults.get(sessionId);
            if (!(pending && pending.paid) && !(existing && existing.paid)) {
                let checkout;
                try {
                    checkout = await stripe.checkout.sessions.retrieve(sessionId);
                } catch (e) {
                    return res.status(404).json({ status: 'not_found', message: 'Sitzung nicht gefunden.' });
                }
                if (checkout.payment_status !== 'paid') {
                    return res.status(402).json({ status: 'unpaid', message: 'Zahlung noch nicht bestätigt.' });
                }
                if (pending) pending.paid = true;
                if (existing) existing.paid = true;
            }
            // Bezahlt → Analyse jetzt starten (einmalig)
            if (pending && !pending.started && !existing) {
                pending.started = true;
                console.log('💳 Zahlung bestätigt – starte Analyse für', sessionId);
                analyzeCareerWithAI(pending.formData, sessionId)
                    .then(() => { const r = analysisResults.get(sessionId); if (r) r.paid = true; })
                    .catch(err => { console.error('Analysis error:', err); pending.started = false; });
            }
        } else if (!sessionId.startsWith('partner_')) {
            return res.status(404).json({ status: 'not_found' });
        }

        const result = analysisResults.get(sessionId);

        if (!result && fehlgeschlageneAnalysen.has(sessionId) && !(pendingPaidAnalyses.get(sessionId) || {}).started) {
            return res.status(200).json({ status: 'error', message: 'Die Analyse konnte nicht erstellt werden.' });
        }
...
// CHATBOT ENDPOINT
// ==========================================
app.post('/api/chatbot', rateLimit('chatbot', 200, HOUR), async (req, res) => {
    try {
        let { question, analysisContext, sessionId } = req.body;
        question = String(question || '').slice(0, 1000);
        analysisContext = String(analysisContext || '').slice(0, 40000);
        
        console.log('💬 Chatbot-Frage erhalten (Inhalt wird aus Datenschutzgründen nicht protokolliert)');
...
app.post('/berufslexikon', rateLimit('lexikon', 100, HOUR), async (req, res) => {
    try {
...
- Seiten-Wrapper: font-family:'Work Sans',sans-serif; max-width:1200px; margin:0 auto; padding:0 16px; box-sizing:border-box;
- MOBILE-PFLICHT: Alle Elemente MÜSSEN auf 320px Bildschirmbreite passen!
- NIEMALS body, html oder :root Styles setzen – das beeinflusst die übergeordnete Seite!
- NIEMALS max-width auf dem äußersten Container größer als 100% setzen!
- Der gesamte HTML-Output wird in ein div eingebettet – kein DOCTYPE, kein html/head/body Tag!
- NIEMALS feste Breiten in px verwenden – nur % oder max-width!
- Alle divs: box-sizing:border-box; width:100%; overflow:hidden;
- Tabellen: width:100%; table-layout:fixed; word-wrap:break-word;
- Grids: display:grid; grid-template-columns:repeat(auto-fit,minmax(380px,1fr)); gap:12px;
- Kein overflow-x auf der Seite!
...
<a href="/" style="display:inline-block;background:linear-gradient(135deg,#c9a84c,#e8c96a);color:#0f1f3d;padding:14px 32px;border-radius:8px;font-weight:700;text-decoration:none;font-size:1rem;">Jetzt KI-Analyse starten – nur 9,99€</a>
...
// Start server
// Partner Kontakt Endpoint
app.post('/partner-kontakt', rateLimit('kontakt', 20, HOUR), async (req, res) => {
  const { unternehmen, ansprechpartner, email, telefon, paket, nachricht } = req.body;
  ...
  try {
    ...
    await resend.emails.send({
      from: 'Dein Karriereweg <onboarding@resend.dev>',
      to: 'denny1403@gmx.de',
      subject: `Neue Partneranfrage: ${unternehmen}`,
      html: `
...
app.post('/feedback', async (req, res) => {
    try {
        ...
        if (!sessionId || !rating) {
            return res.status(400).json({ error: 'sessionId und rating sind erforderlich' });
        }
        if (!['up', 'down'].includes(rating)) {
            return res.status(400).json({ error: 'rating muss "up" oder "down" sein' });
        }
        if (!dbPool) {
            console.warn('⚠️ Feedback erhalten, aber keine Datenbank verbunden:', sessionId, rating);
            return res.status(503).json({ error: 'Feedback-Speicherung aktuell nicht verfügbar' });
        }

        await dbPool.query(
            'INSERT INTO feedback (session_id, rating) VALUES ($1, $2)',
            [sessionId, rating]
        );

        console.log(`📝 Feedback gespeichert: ${sessionId} -> ${rating}`);
        res.json({ success: true });
    } catch (error) {
...
// Kleine Auswertung: Gesamtquote abrufen (z.B. für dich selbst zum Nachschauen)
app.get('/feedback/stats', async (req, res) => {
    try {
        if (!dbPool) return res.status(503).json({ error: 'Keine Datenbank verbunden' });
        const result = await dbPool.query(`
            SELECT rating, COUNT(*) as count FROM feedback GROUP BY rating
        `);
        res.json({ stats: result.rows });
    } catch (error) {
        console.error('❌ Feedback-Stats-Fehler:', error);
...
// ═══════════════════════════════════════════════
// SPONSOR-KLICK-TRACKING
// ═══════════════════════════════════════════════
app.post('/track-sponsor-click', async (req, res) => {
    try {
        const { company, placement } = req.body;
        const sessionId = req.body.sessionId || null;

        if (!company || typeof company !== 'string') {
            return res.status(400).json({ error: 'company ist erforderlich' });
        }
        if (!dbPool) {
            console.warn('⚠️ Sponsor-Klick erhalten, aber keine Datenbank verbunden:', company);
            return res.status(503).json({ error: 'Tracking aktuell nicht verfügbar' });
        }

        await dbPool.query(
            'INSERT INTO sponsor_clicks (company_slug, placement, session_id) VALUES ($1, $2, $3)',
            [company, placement || null, sessionId]
        );

        console.log(`💰 Sponsor-Klick: ${company} (${placement || 'unbekannt'})`);
        res.json({ success: true });
    } catch (error) {
        console.error('❌ Sponsor-Klick-Fehler:', error);
        res.status(500).json({ error: 'Klick konnte nicht gespeichert werden' });
    }
});

// Monats-Auswertung pro Firma, z.B. /sponsor-clicks/stats?company=musterfirma&month=2026-09
app.get('/sponsor-clicks/stats', async (req, res) => {
    try {
        if (!dbPool) return res.status(503).json({ error: 'Keine Datenbank verbunden' });

        const { company, month } = req.query;
        let query = 'SELECT company_slug, placement, COUNT(*) as clicks FROM sponsor_clicks WHERE 1=1';
        const params = [];

        if (company) {
            params.push(company);
            query += ` AND company_slug = $${params.length}`;
        }
        if (month) {
            params.push(month); // Format: 'YYYY-MM'
            query += ` AND TO_CHAR(created_at, 'YYYY-MM') = $${params.length}`;
        }
        query += ' GROUP BY company_slug, placement ORDER BY company_slug, placement';

        const result = await dbPool.query(query, params);
        res.json({ stats: result.rows });
    } catch (error) {
        console.error('❌ Sponsor-Stats-Fehler:', error);
        res.status(500).json({ error: 'Stats konnten nicht geladen werden' });
    }
});
...
// ═══════════════════════════════════════════════
// BILDUNGSWEG-MINI-ANALYSE (schnelle Einzelfrage,
// kein Karriereweg-Report – nur "welcher Schulweg passt")
// ═══════════════════════════════════════════════
app.post('/bildungsweg-check', rateLimit('bildungsweg', 60, HOUR), async (req, res) => {
    try {
        const { schulform, noten, praktisch, studium } = req.body;

        if (!schulform || !noten || !praktisch || !studium) {
            return res.status(400).json({ error: 'Alle Felder sind erforderlich' });
        }

        const prompt = `Du bist ein erfahrener, freundlicher Bildungsberater für Schüler nach der 10. Klasse in Deutschland.

Angaben der Schülerin/des Schülers:
- Aktuelle Schulform: ${schulform}
- Notenschnitt (grob): ${noten}
- Arbeitet lieber: ${praktisch}
- Studium später gewünscht: ${studium}

Empfiehl GENAU EINEN der folgenden vier Wege, der am besten passt: "Ausbildung", "Fachoberschule (Fachabitur)", "Gymnasiale Oberstufe (Abitur)", "Berufsfachschule".

Antworte AUSSCHLIESSLICH mit einem validen JSON-Objekt in genau diesem Format, ohne Zusatztext, ohne Markdown-Codeblock:
{"empfehlung": "<einer der 4 Wege, exakt wie oben geschrieben>", "begruendung": "<2-3 einfache, verständliche Sätze für Schüler UND Eltern, warum dieser Weg passt>", "alternative": "<1 Satz: welcher andere Weg auch infrage käme und warum>"}`;

        const completion = await callWithRetry(() => openai.chat.completions.create({
            model: "gpt-4o",
                temperature: 0.3,
            messages: [
                { role: "system", content: "Du bist ein präziser, hilfreicher Bildungsberater. Du antwortest immer nur im geforderten JSON-Format." },
                { role: "user", content: prompt }
            ],
            max_tokens: 350,
        }));
        ...
        if (!parsed.empfehlung || !parsed.begruendung) {
        ...
        console.error('❌ Bildungsweg-Check-Fehler:', error);
        res.status(500).json({ error: 'Die Einschätzung konnte nicht erstellt werden. Bitte versuch es nochmal.' });
    }
});
...
// ═══════════════════════════════════════════════
// LEBENSLAUF- & ANSCHREIBEN-GENERATOR
// ═══════════════════════════════════════════════
app.post('/lebenslauf-generator', rateLimit('lebenslauf', 60, HOUR), async (req, res) => {
    try {
        const {
            vorname, nachname, strasse, geburtsdatum, staatsangehoerigkeit, stadt, email, telefon,
            schule, abschluss, notendurchschnitt,
            praktika, faehigkeiten, sprachen, hobbys, zielberuf,
            firmenname, ansprechpartner, firmenadresse, referenznummer, warumUnternehmen, verfuegbarAb
        } = req.body;

        if (!vorname || !nachname || !schule || !abschluss || !zielberuf) {
            return res.status(400).json({ error: 'Pflichtfelder fehlen (Name, Schule, Abschluss, Zielberuf)' });
        }

        console.log(`📄 Lebenslauf-Generator angefragt → ${zielberuf}`);

        const anrede = ansprechpartner
            ? `Sehr geehrte(r) ${ansprechpartner}`
            : 'Sehr geehrte Damen und Herren';

        const prompt = `Du bist ein erfahrener Bewerbungscoach für Ausbildungsplätze in Deutschland. Erstelle auf Basis folgender Angaben einen professionellen, tabellarischen Lebenslauf UND ein passendes, personalisiertes Anschreiben für eine Bewerbung als "${zielberuf}"${firmenname ? ` bei "${firmenname}"` : ''}.

ANGABEN ZUR PERSON:
- Name: ${vorname} ${nachname}
- Adresse: ${strasse || 'nicht angegeben'}, ${stadt || 'nicht angegeben'}
- Geburtsdatum: ${geburtsdatum || 'nicht angegeben'}
- Staatsangehörigkeit: ${staatsangehoerigkeit || 'nicht angegeben'}
- E-Mail: ${email || 'nicht angegeben'}
- Telefon: ${telefon || 'nicht angegeben'}
- Schule(n): ${schule}
- Abschluss: ${abschluss}
- Notendurchschnitt: ${notendurchschnitt || 'nicht angegeben'}
- Praktika/Berufserfahrung: ${praktika || 'keine angegeben'}
- Besondere Fähigkeiten/EDV-Kenntnisse: ${faehigkeiten || 'keine angegeben'}
- Sprachen: ${sprachen || 'keine angegeben'}
- Hobbys/Interessen: ${hobbys || 'keine angegeben'}

ANGABEN ZUM ZIELUNTERNEHMEN (für das Anschreiben):
- Ziel-Ausbildungsberuf: ${zielberuf}
- Firmenname: ${firmenname || 'nicht angegeben – allgemein halten'}
- Ansprechpartner: ${ansprechpartner || 'nicht angegeben – "Sehr geehrte Damen und Herren" verwenden'}
- Firmenadresse: ${firmenadresse || 'nicht angegeben'}
- Referenz-/Stellennummer: ${referenznummer || 'nicht angegeben'}
- Warum dieses Unternehmen (Motivation des Bewerbers): ${warumUnternehmen || 'nicht angegeben – allgemein auf Branche/Beruf beziehen'}
- Verfügbar ab: ${verfuegbarAb || 'nicht angegeben – nicht erwähnen'}

WICHTIGE REGELN:
- Erfinde NIEMALS Fakten (keine erfundenen Praktika, Noten, Fähigkeiten oder Motivations-Gründe) – nutze NUR die gegebenen Angaben. Wenn etwas fehlt, lass es weg statt es zu erfinden.
- Anrede im Anschreiben: "${anrede}" verwenden.
- Falls ein Firmenname angegeben ist, MUSS er im Anschreiben konkret erwähnt werden (nicht nur "Ihr Unternehmen").
- Falls "Warum dieses Unternehmen" angegeben ist, MUSS dieser Grund glaubwürdig in einen eigenen Absatz eingebaut werden.
- Falls "Verfügbar ab" angegeben ist, am Ende des Anschreibens kurz erwähnen.
- Professioneller, aber jugendlicher, authentischer Ton – keine übertriebenen Floskeln.
- Tabellarischer Lebenslauf im deutschen Standard-Format (Persönliche Daten, Schulbildung, Praktische Erfahrungen, Kenntnisse & Fähigkeiten, Hobbys).

Antworte AUSSCHLIESSLICH mit einem validen JSON-Objekt in genau diesem Format, ohne Zusatztext, ohne Markdown-Codeblock:
{
  "lebenslauf": {
    "persoenlicheDaten": [{"label": "...", "wert": "..."}],
    "schulbildung": [{"zeitraum": "...", "beschreibung": "..."}],
    "praktischeErfahrungen": [{"zeitraum": "...", "beschreibung": "..."}],
    "kenntnisseFaehigkeiten": ["...", "..."],
    "hobbys": ["...", "..."]
  },
  "anschreiben": {
    "betreff": "Bewerbung um einen Ausbildungsplatz als ${zielberuf}${referenznummer ? ' (Ref. ' + referenznummer + ')' : ''}",
    "text": "<vollständiger Anschreiben-Text mit Anrede, 3-4 Absätzen, Grußformel>"
  }
}`;
        ...
        const completion = await callWithRetry(() => openai.chat.completions.create({
            ...
            temperature: 0.4,
            messages: [
                { role: "system", content: "Du bist ein präziser, ehrlicher Bewerbungscoach. Du erfindest niemals Fakten und antwortest ausschließlich im geforderten JSON-Format." },
                { role: "user", content: prompt }
            ],
            max_tokens: 3000,
        }));
        ...
        if (!parsed.lebenslauf || !parsed.anschreiben) {
            throw new Error('Unvollständige KI-Antwort');
        }

        console.log('✅ Lebenslauf & Anschreiben generiert');
        res.json(parsed);
    } catch (error) {
        console.error('❌ Lebenslauf-Generator-Fehler:', error);
        res.status(500).json({ error: 'Lebenslauf konnte nicht erstellt werden. Bitte versuch es nochmal.' });
    }
});
...
// ═══════════════════════════════════════════════
// SEITENBESUCHE (Page-View-Tracking)
// ═══════════════════════════════════════════════
app.post('/track-visit', async (req, res) => {
    try {
        ...
        await dbPool.query(
            ...
            [path.slice(0, 500), sessionId]
        );

        res.json({ success: true });
    } catch (error) {
        console.error('❌ Visit-Tracking-Fehler:', error);
        res.status(500).json({ error: 'Besuch konnte nicht gespeichert werden' });
    }
});

// Auswertung, z.B. /visits/stats?month=2026-09
app.get('/visits/stats', async (req, res) => {
    try {
        if (!dbPool) return res.status(503).json({ error: 'Keine Datenbank verbunden' });

        const { month, path } = req.query;
        let query = 'SELECT path, COUNT(*) as visits FROM page_visits WHERE 1=1';
        const params = [];

        if (path) {
            params.push(path);
            query += ` AND path = $${params.length}`;
        }
        if (month) {
            params.push(month);
            query += ` AND TO_CHAR(created_at, 'YYYY-MM') = $${params.length}`;
        }
        query += ' GROUP BY path ORDER BY visits DESC';

        const result = await dbPool.query(query, params);
        const totalResult = await dbPool.query(
            month ? `SELECT COUNT(*) as total FROM page_visits WHERE TO_CHAR(created_at, 'YYYY-MM') = $1` : 'SELECT COUNT(*) as total FROM page_visits',
            month ? [month] : []
        );

        res.json({ stats: result.rows, total: totalResult.rows[0].total });
    } catch (error) {
        console.error('❌ Visits-Stats-Fehler:', error);
        res.status(500).json({ error: 'Stats konnten nicht geladen werden' });
    }
});
